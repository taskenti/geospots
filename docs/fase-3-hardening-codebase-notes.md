...
**Próximo paso:** Sprint 1 ✅ completo (T1.1 + T1.2 + T1.3). Siguiente bloque: **Sprint 2** — migración v6 monolítica (`db/migration_phase3_v6.sql`) cubriendo T1.4 (`spot_alerts`), T1.4b (`spot_function`/`is_overnight_viable`/`authorization_status` + `spot_geo.source`), T1.4c (`signal_flux` + `active_alert_types` en `spot_semantic_state`), T1.5 (`canonical_tags` + `unknown_tags`), T1.6 (`semantic_fingerprint` + `built_from_fingerprint`), T1.8 (`llm_call_metrics`), T2.7 (trigger `stale`). Ampliar SYSTEM_PROMPT_V2 con el nuevo schema (`alerts[]`, `spot_function`, `is_overnight_viable`, `authorization_status`, `elevation_m`, `terrain_type`, `slope_degrees`). Crear `enrichment/state_resolver.py` (decay) y `enrichment/tag_canonicalizer.py`. Cuando este sprint cambie el schema de output bumpar `PROMPT_VERSION` (probablemente a `v6-*`) — ENRICHMENT_VERSION queda en 5 si los campos son top-level adicionales no destructivos, o bumpar a 6 si los emisores aguas abajo lo prefieren.
