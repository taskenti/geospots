...
**Próximo paso:** T1.5 — `tag_canonicalizer.py` + tablas `canonical_tags` + `unknown_tags`. Necesita migración complementaria (puede ir en `db/migration_phase3_v6b.sql` o append al v6). Archivos: nueva `enrichment/tag_canonicalizer.py` con `canonicalize_tag(raw) -> canonical_id | None`; `ingest_v2._update_narrative_and_materialized` debe filtrar `parsed.tags` por canonicalización antes del UPDATE; `unknown_tags` se incrementa cuando devuelve None. Job opcional `jobs/review_unknown_tags.py` (no bloqueante pre-batch, puede quedar en Tier 2 si el tiempo aprieta).
